...
A new vault ships with every optional capability disarmed. With nothing armed, OpenBrain reads nothing outside the vault root, writes nothing but its own local index, and sends nothing anywhere. Everything below describes what changes once you arm something, one capability at a time, and how to check or undo that choice.
...
| `classifier` | Staged candidates already in the vault. Never a transcript directly, never the preference kernel. | The text of the staged candidates it is given, to whichever model provider your host CLI is configured against. The only capability that spends money or leaves the machine. |
...
| `learning.consolidate` | The belief store, the decision journal, and the confidence values written by the evaluator. | Nothing. It deletes rather than sends. |
...
**How to erase what was derived.** `open-brain transcripts purge` deletes every candidate ever derived from a transcript, and names which ones. Add `--include-prompt` to also delete candidates captured from a submitted prompt, `--dry-run` to see the list first, and `--yes` to actually delete. One limit on "erase everything": a candidate you already decided on through `open-brain sync` lives in a monthly archive under `10_memory/staging/archive/`, which is the record of what you approved or rejected, and purge does not touch that archive. Removing it means deleting the archive file itself.

## What can leave this machine

Of the seven capabilities, exactly one sends anything off this machine: `classifier`. When armed, it sends the text of staged candidates, never a raw transcript and never the preference kernel directly, to whichever model provider your host CLI is configured against, billed by that provider under its own terms. Calls are capped by `capabilities.classifier.daily_call_budget` (25 per day by default), and a run stops when the budget is spent.
